    ...
    <main className="grow pt-20">
      ...
      <section className="py-24 bg-olive-950 text-white">
        <div className="max-w-4xl mx-auto px-6 text-center">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            <span className="text-terracotta font-bold uppercase tracking-widest text-sm mb-4 block">{TEXTS.b2b.hero.tagline}</span>
            <h1 className="text-5xl md:text-6xl mb-8 leading-[1.1]">
              {TEXTS.b2b.hero.headline}<br />
              <span className="italic text-olive-300">{TEXTS.b2b.hero.headlineItalic}</span>
            </h1>
            <p className="text-xl text-olive-300 leading-relaxed max-w-2xl mx-auto">
              {TEXTS.b2b.hero.subtext}
            </p>
          </motion.div>
        </div>
      </section>
      ...
      <section className="py-24 bg-white">
        <div className="max-w-7xl mx-auto px-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            <div>
              <h2 className="text-4xl md:text-5xl mb-8">{TEXTS.b2b.why.headline}</h2>
              <p className="text-olive-700 mb-8 leading-relaxed">
                {TEXTS.b2b.why.subtext}
                <strong className="text-charcoal"> {TEXTS.b2b.why.subtextBold}</strong>
              </p>
              ...
              <ul className="space-y-4">
                {TEXTS.b2b.benefits.map((benefit, i) => (
      ...
      <section id="kontakt" className="py-24 bg-white scroll-mt-20">
        <div className="max-w-4xl mx-auto px-6">
          <h2 className="text-4xl md:text-5xl mb-4 text-center">{TEXTS.b2b.form.headline}</h2>
          <p className="text-olive-600 text-center mb-16 max-w-xl mx-auto">
            {TEXTS.b2b.form.subtext}
          </p>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-12">
            {isSuccess ? (
              ...
              >
                ...
                <h3 className="text-2xl font-serif text-charcoal mb-4">{TEXTS.b2b.form.successHeadline}</h3>
                <p className="text-olive-700 leading-relaxed">{TEXTS.b2b.form.successText}</p>
              </motion.div>
            ) : (
              <form onSubmit={handleSubmit(onSubmit)} className="lg:col-span-3 space-y-6">
                ...
                <div>
                  ...
                  <select {...register('volume')} id="volume" className="w-full px-4 py-3 rounded-xl border border-olive-200 bg-olive-50 focus:outline-none focus:ring-1 focus:ring-olive-300 focus:border-olive-400 cursor-pointer transition-shadow">
                ...
                <div>
                  ...
                  <textarea {...register('message')} id="message" rows={4} className="w-full px-4 py-3 rounded-xl border border-olive-200 bg-olive-50 focus:outline-none focus:ring-1 focus:ring-olive-300 focus:border-olive-400 resize-none transition-shadow" />
                </div>
                {submitError && (
                  <p className="text-red-500 text-sm text-center">
                    Fehler beim Senden. Bitte versuchen Sie es erneut oder schreiben Sie uns direkt an{' '}
                    <a href={`mailto:${COMPANY_INFO.contact.emailB2B}`} className="underline">{COMPANY_INFO.contact.emailB2B}</a>.
                  </p>
                )}
                <button
                  type="submit"

import { Droplets, ShieldCheck, Instagram, Facebook, Twitter } from 'lucide-react';
    ...
    <footer className="bg-olive-950 text-olive-200 py-20">
      <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12">
        <div>
          <div className="flex items-center gap-2 mb-6">
            ...
            <span className="text-xl font-serif font-bold tracking-tight text-white">{COMPANY_INFO.name}</span>
          </div>
          <p className="text-sm leading-relaxed mb-6">
            {COMPANY_INFO.tagline} Nachhaltig, fair und von höchster Güteklasse.
          </p>
          <div className="flex gap-4">
            <a href={COMPANY_INFO.social.instagram} className="w-10 h-10 bg-white/5 rounded-full hover:bg-harvest-gold hover:text-white transition-all duration-300 flex items-center justify-center cursor-pointer shadow-sm hover:-translate-y-1">
              <Instagram size={18} />
            </a>
            <a href={COMPANY_INFO.social.facebook} className="w-10 h-10 bg-white/5 rounded-full hover:bg-harvest-gold hover:text-white transition-all duration-300 flex items-center justify-center cursor-pointer shadow-sm hover:-translate-y-1">
              <Facebook size={18} />
            </a>
            <a href={COMPANY_INFO.social.twitter} className="w-10 h-10 bg-white/5 rounded-full hover:bg-harvest-gold hover:text-white transition-all duration-300 flex items-center justify-center cursor-pointer shadow-sm hover:-translate-y-1">
              <Twitter size={18} />
            </a>
          </div>
        </div>
        ...
        <div>
          <h4 className="text-white font-bold mb-6">{COMPANY_INFO.name}</h4>
          <ul className="space-y-4 text-sm">
      ...
      <div className="max-w-7xl mx-auto px-6 pt-16 mt-16 border-t border-white/10 flex flex-col md:flex-row justify-between items-center gap-8 text-xs text-olive-400">
        <div className="flex flex-col md:flex-row items-center gap-4 md:gap-8 order-2 md:order-1">
          <p>© {year} {COMPANY_INFO.name}. Alle Rechte vorbehalten.</p>
